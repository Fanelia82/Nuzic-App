# App Nuzic

This project has a clear objective: to renew the way we explain music, in order to offer an open and fresh perspective that facilitates the learning of existing musical theory, and serves as a breeding ground for generating new ideas, opening new horizons in musical composition.

https://www.nuzic.org
https://www.nuzic.org/App

Community
https://discord.gg/zMpCwk5w**

## Contributing

Nuzic App is being developed on **[GitLab](https://gitlab.com/nuzic/nuzic_app)**, a development platform that is itself part of the open source community and that can be self-hosted, if the need arises.
If you want to collaborate feel free to join us on our Discord server

https://discord.gg/zMpCwk5w**
